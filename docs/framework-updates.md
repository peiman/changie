...
2. **Sets up remote** — Adds or reuses the `ckeletin-upstream` git remote pointing to `https://github.com/peiman/changie.git`
...
When you fork ckeletin-go and run `task init`, your module path changes (e.g., from `github.com/peiman/changie` to `github.com/you/myapp`). Framework code in `.ckeletin/pkg/` imports need to reflect your module path, not the upstream one.
...
This is fundamentally safer than `sed`-based string replacement because it understands Go syntax. A string like `"github.com/peiman/changie"` in a comment or test fixture won't be incorrectly rewritten.
