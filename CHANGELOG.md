...
- Log noise from init command leaking to console
- Changelog template trailing blank line causing validation failure

## [v1.2.0] - 2026-04-13

### Added

- Ported to [ckeletin-go](https://github.com/peiman/ckeletin-go) v2 scaffold for improved architecture, testability, and developer tooling
- `changie changelog validate` command — validates changelog for common problems (version headers, duplicates, broken links, missing dates, semver order)
- `changie diff FROM TO` command — shows changelog entries between two versions
- JSON output mode (`--output json`) with standard envelope for machine-readable output
- XDG Base Directory support for configuration file paths
- Dual logging system with console and file outputs, including log rotation
...
[0.0.0]: https://github.com/peiman/changie/releases/tag/v0.0.0